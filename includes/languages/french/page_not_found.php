<?php
/**
 * @package languageDefines
 * @copyright Copyright 2003-2005 Zen Cart Development Team
 * @copyright Portions Copyright 2003 osCommerce
 * @license http://www.zen-cart.com/license/2_0.txt GNU Public License V2.0
 * @version $Id: page_not_found.php 3159 2006-03-11 01:35:04Z drbyte $
 * 
 * Traduction française de Zen Cart 1.3.9 par Zen Cart France. 
 * Auteur : Damien Desrousseaux : http://www.zencart-france.com 
 * Package : zen-cart-v1.3.9-FR
 */

define('NAVBAR_TITLE', 'Page Non trouv&eacute;e');
define('HEADING_TITLE', 'Page Non trouv&eacute;e');
define('TEXT_INFORMATION', '');
define('PAGE_ACCOUNT', 'Mon compte');
define('PAGE_ACCOUNT_EDIT', 'Information du compte');
define('PAGE_ADDRESS_BOOK', 'Carnet d\'adresses');
define('PAGE_ACCOUNT_HISTORY', 'Historique de commandes');
define('PAGE_ACCOUNT_NOTIFICATIONS', 'Souscriptions &agrave; la newsletter');
define('PAGE_SHOPPING_CART', 'Panier');
define('PAGE_CHECKOUT_SHIPPING', 'Commander');
define('PAGE_ADVANCED_SEARCH', 'Recherche avanc&eacute;e');
define('PAGE_PRODUCTS_NEW', 'Nouveaux produits');
define('PAGE_SPECIALS', 'Promotions');
define('PAGE_REVIEWS', 'Avis');
?>